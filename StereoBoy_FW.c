...
void dac_int_callback(uint gpio, uint32_t events) {
    ...
    if (dac_read(0, 0x2E) & 0x10) { // Bit 5
        printf("Headphones plugged in! Paused and switching to headphones.\n");
        // pause without warping
        paused = 1;
        warping = 0;
    } else {
        printf("Headphones pulled out! Paused and witching to speakers.\n");
        // pause without warping
        paused = 1;
        warping = 0;
    }
...
void dac_interrupt_init(void) {
    ...
    gpio_set_irq_enabled_with_callback(
        15,
        GPIO_IRQ_EDGE_RISE | GPIO_IRQ_EDGE_FALL,   // active-low interrupt
        true,
        &dac_int_callback
    );
}
