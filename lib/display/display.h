...
void lcd_draw_progress_bar(PIO pio, uint sm, int length, int progress, uint16_t * framebuffer){
...
void set_pixel(uint16_t x, uint16_t y, uint16_t color, uint16_t * framebuffer) {
    framebuffer[y * SCREEN_WIDTH + x] = color;
}

void lcd_update(PIO pio, uint sm, uint16_t * framebuffer) {
...
void lcd_draw_circle (uint16_t x, uint16_t y, uint8_t radius, uint16_t color, uint16_t * framebuffer){
    float theta = 0.5;
    uint16_t writex;
    uint16_t writey;
    for (int i = 0; i<360/theta; i++){
        ...
        if (0<=writex && 240>writex && 0<=writey && 240>writey){
            set_pixel(writex, writey, color, framebuffer);
        }
    }
}

void lcd_draw_circle_fill(uint16_t x, uint16_t y, uint8_t radius, uint16_t color, uint16_t * framebuffer){
    float theta = 0.5;
    uint16_t writex;
    uint16_t writey;
...
void lcd_draw_char(uint16_t x, uint16_t y, char c, uint16_t color, uint16_t * framebuffer) {
    const struct Font* f = find_font_char(c);
    if (!f) return;
    for (uint8_t row = 0; row < 7; row++) {
        for (uint8_t col = 0; col < 5; col++) {
            if (f->code[row][col] == '1') {
                set_pixel(x + col, y + row, color, framebuffer);
            }
        }
    }
}

void lcd_draw_string(uint16_t x, uint16_t y, const char *text, uint16_t color, uint16_t * framebuffer) {
    uint16_t start_x = x;
    for (int i = 0; text[i] != '\0'; i++) {
        lcd_draw_char(start_x, y, text[i], color, framebuffer);
        start_x += 6;
    }
}
