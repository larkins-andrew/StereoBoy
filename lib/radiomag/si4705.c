...
/*
void switch_radio_audio_mode(vs1053_t *player, uint16_t current_freq, bool* is_digital_audio, uint8_t fm_vol, uint8_t current_antenna){
...
void print_current_station() {
    ...
    printf("-> Station: %d.%d MHz | RSSI: %3d dBuV | SNR: %3d dB | Valid: %s\n", 
           ...
           rssi, snr, 
           valid ? "YES" : "NO");
    // dprint("Station: %d.%d", freq/100, freq%100);
}