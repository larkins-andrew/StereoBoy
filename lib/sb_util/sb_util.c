...
void core1_entry() {
    // We need to pass the display config somehow. 
    // Since we initialized it on Core 0, we can just use the global pointer
    // or pass it through the multicore FIFO.
    
    // Core 1 needs its own ADC setup if not already global
    adc_select_input(ADC_CH);

    while (1) {
        // High-speed sampling loop
        // Because Core 1 has NOTHING else to do, this will be buttery smooth.
